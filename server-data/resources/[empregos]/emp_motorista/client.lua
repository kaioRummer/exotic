				...
				if distance <= 2.5 then
					...
					if IsControlJustPressed(0,38) then
						if IsVehicleModel(GetVehiclePedIsUsing(PlayerPedId()),GetHashKey("coach")) or IsVehicleModel(GetVehiclePedIsUsing(PlayerPedId()),GetHashKey("bus")) then
							RemoveBlip(blip)
							TriggerEvent("vrp_sound:source",'moeda',0.7)
							if destino == 27 then
...
Citizen.CreateThread(function()
    while true do
        ...
        local Distancia = GetDistanceBetweenCoords(Coordenadas.x, Coordenadas.y, Coordenadas.z, 453.82388305664,-600.67486572266,28.586614608765, true) 
        if Distancia < 20.0 then
            Opacidade = math.floor(255 - (Distancia * 2))
            Texto3D(453.82388305664,-600.67486572266,28.586614608765, "CARROS", Opacidade)
            ---DrawMarker(36,453.82388305664,-600.67486572266,28.586614608765, 0, 0, 0, 0, 0, 270, 1.1001, 1.1001, 1.1001, 1.1001, 1.1001, 1.1001, 153, 204,50, Opacidade, 0, 0, 0, 0)
        end
    end
end)
...
function Texto3D(x,y,z, text, Opacidade)
    ...
    if onScreen then
        ...
        SetTextColour(0, 0, 0, Opacidade)
        SetTextDropshadow(0, 0, 0, 0, Opacidade)
        SetTextEdge(10, 0, 0, 138, 300)
        SetTextDropShadow()
        SetTextOutline()
        SetTextEntry("STRING")
        SetTextCentre(1)
...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(1)
        local Coordenadas = GetEntityCoords( GetPlayerPed(-1) )
        local Distancia = GetDistanceBetweenCoords(Coordenadas.x, Coordenadas.y, Coordenadas.z, 453.48, -607.74, 28.57, true) 
        if Distancia < 20.0 then
            Opacidade = math.floor(255 - (Distancia * 2))
            --Texto3D(-37.430976867676, -1100.5216064453, 26.122351837158, "CARROS", Opacidade)
            DrawMarker(20, 453.48, -607.74, 28.57, 0, 0, 0, 0, 0, 270, 0.800, 0.800, 0.800, 0.800, 0.800, 0.800, 153, 204,50, Opacidade, 0, 0, 0, 0)
        end
    end
end)
...
local TeleportFromTo = {
	["texto_localonibus"] = {
		positionFrom = { ['x'] =  461.49, ['y'] = -603.424, ['z'] = 28.49, nom = "~h~~r~Atenção~n~"},  
		positionTo = {  ['x'] =  461.49, ['y'] = -603.424, ['z'] = 27.49,nom = "~b~Manhenta sempre está área livre~r~ \n~y~para evitar acitendes~y~"},
	},
}
