...
Citizen.CreateThread(
	function()
		...
		while true do
			...
			if
				GetDistanceBetweenCoords(-342.16, -1560.07, 25.24, GetEntityCoords(GetPlayerPed(-1))) <
					10.0
			 then
				DrawMarker(23,-342.16, -1560.07, 25.24 - 0.9001,0,0,0,0,0,0,10.0,10.0,1.0,0,95,140,50,0,0,0,0)
			end
...
function openMenuTrash(nearTrash, trashModel)
	if nearTrash then
		drawTxt("PRESSIONE ~g~E~s~ PARA COLETAR O LIXO", 1, 1, 0.5, 0.8, 0.6, 255, 255, 255, 255)
...
function openMenuTrashCar(nearTrashCar)
	if nearTrashCar and CanLeaveBox then
		drawTxt("PRESSIONE ~g~Z~s~ PARA GUARDAR O LIXO", 1, 1, 0.5, 0.8, 0.6, 255, 255, 255, 255)
