...
-----------------------------------------------------------------------------------------------------------------------------------------
-- VARIAVEIS       758.11, -1332.42, 27.28
-----------------------------------------------------------------------------------------------------------------------------------------
local blips = false
local random = 0
local modules = ""
...
local CoordenadaX = 758.11
local CoordenadaY = -1332.42
local CoordenadaZ = 27.28
...
RegisterCommand("carga",function(source,args)
	...
	if distance <= 50.1 and not servico then
		...
		elseif args[1] == "caminhao" then
			servico = true
			modules = args[1]
			servehicle = -1770643266
			random = emP.getTruckpoint(modules)
			CoordenadaX2 = caminhao[random].x
			CoordenadaY2 = caminhao[random].y
			CoordenadaZ2 = caminhao[random].z
			CriandoBlip(CoordenadaX2,CoordenadaY2,CoordenadaZ2)
			TriggerEvent("Notify","importante","Entrega de <b>caminhaos</b> iniciada, pegue o caminhão, a carga e vá até o destino marcado.")
		else
			TriggerEvent("Notify","aviso","<b>Disponíveis:</b> diesel, cars, caminhao, woods e gas")
		end
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
