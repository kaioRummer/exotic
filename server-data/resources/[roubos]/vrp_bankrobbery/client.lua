        ...
        if not robberyOngoing then
            for bank,values in pairs(BankHeists) do
                ...
                if distanceCheck <= 1.0 then
                   drawTxt("PRESSIONE  ~r~E~w~  PARA HACKEAR",4,0.5,0.90,0.50,255,255,255,200)
...
function HackAnimStart(x,y,z,h)
    ...
    laptop = CreateObject(("prop_laptop_01a"),x-0.4,y+0.2,z-1,true,true,true)
...
function StartRobbery(bankId)
    ...
    CashPile = CreateObject(("bkr_prop_bkr_cashpile_04"),CashPosition["x"],CashPosition["y"],CashPosition["z"],false)
    ...
    Citizen.CreateThread(function()
        while robberyOngoing do
            ...
            if distanceCheck <= 1.5 then
                drawTxt("PRESSIONE  ~r~E~w~  PARA ~y~PEGAR O DINHEIRO",4,0.5,0.90,0.50,255,255,255,200)
                if IsControlJustPressed(0,38) then
                    TriggerEvent("progress",7500,"Pegando o Dinheiro")
                    GrabCash(bankId)
                end
            end
        end
