...
-- WEBHOOK
-----------------------------------------------------------------------------------------------------------------------------------------
local webhookbankrobbery = "https://discordbhooks/836807279231762512/9hpFjwEfhrWQjxqteYiyXlTdT5k9j1VbnyHvLm2ynuCao6ezT8jSKiUABvS375HB2NBT"
...
function func.CheckPayment(bankId)
    ...
    if user_id then
        if parseInt(moneyHeist[bankId]) <= parseInt(BankHeists[bankId]["Money"]) then
            ...
            TriggerClientEvent("Notify",source,"sucesso","Recebeu <b>$"..vRP.format(parseInt(cashRecieved)).." reais sujos</b>.")
        else
...
function func.CheckPolice()
    ...
    if user_id then
        ...
        if #policia < -1 then --<= 0 then --3 then
        ...
        for l,w in pairs(policia) do
            ...
            if player then
                async(function()
                    ...
                    TriggerClientEvent("NotifyPush",player,{ code = 31, title = "O roubo começou no Banco Central, dirija-se até o local e intercepte os assaltantes.", x = x, y = y, z = z, badge = "Banca Central." })
