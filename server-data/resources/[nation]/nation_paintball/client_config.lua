...
config = {
    ...
    startcoords = vec3(158.42,-994.38,29.36),
    ...
    startRound = function() -- funcao executada toda vez que o round é iniciado
        local ped = PlayerPedId()
        local weapon = sv_config.defaultWeapon
        local weapons = {
            [weapon] = { ammo = 250 }
        }

        if not oldWeapons then
            oldWeapons = vRP.replaceWeapons(weapons) or {} -- salvar armas antigas
            myArmour = GetPedArmour(ped) -- salvar colete
            vRP.setArmour(0)   -- remover colete
        else
            local canRefill = function(weapon)
                for k,v in ipairs(config.weaponStore) do
                    if v.weapon == weapon and v.ammo == 250 then
                        return true
                    end
                end
                return false
    ...
    reset = function() -- função de resetar a partida
        ...
        vRP._stopAnim(false)
        vRP.replaceWeapons(oldWeapons or {})
        oldWeapons = false
...
CreateThread(function()
    ...
    config.defaultWeapon = sv_config.defaultWeapon
...
CreateThread(function()
    while true do
        local idle = 1000
        if inLobby and not inGame then
            idle = 0
