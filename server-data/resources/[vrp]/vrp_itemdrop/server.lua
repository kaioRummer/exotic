...
-- WEBHOOK
-----------------------------------------------------------------------------------------------------------------------------------------
local webhookpegaritem = "https://canary.discord.com/api/webhooks/876020767077306398/EsUYLTkwBw4vau75DAQswuU50fE4ffZ6d2_k5gtRZkX-mQp0QKcyGUOXKhTajHDXFV2m"
	...
	if user_id then
		if items[id] ~= nil then
			...
			if new_weight <= vRP.getInventoryMaxWeight(user_id) then
				...
				local teste = items[id]
				items[id] = nil
				vRP.giveInventoryItem(user_id,teste.item,teste.count)
				vRPclient._playAnim(source,true,{{"pickup_object","pickup_low"}},false)
				local identity = vRP.getUserIdentity(user_id)
				SendWebhookMessage(webhookpegaritem,"```prolog\n[ID]: "..user_id.." "..identity.name.." "..identity.firstname.." \n[PEGOU]: "..teste.name.." \n[QUANTIDADE]: "..teste.count.." "..os.date("\n[Data]: %d/%m/%Y [Hora]: %H:%M:%S").." \r```")
				markers_ids:free(id)
