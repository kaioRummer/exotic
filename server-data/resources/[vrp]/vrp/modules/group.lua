...
function vRP.getGroupType(group)
        local g = groups[group]
        if g and g._config and g._config.gtype then
            return g._config.gtype
        end
...
function vRP.getUsersByPermission(perm)
	...
	for k,v in pairs(vRP.rusers) do
		if vRP.terPemissao(tonumber(k),perm) then
...
vRP.registerPermissionFunction("not",function(user_id,parts)
	return not vRP.terPemissao(user_id,"!"..table.concat(parts,".",2))
end)
...
function vRP.terPemissao(user_id,perm)
	local user_groups = vRP.getUserGroups(user_id)
...
function vRP.terPemissaos(user_id, perms)
	for k,v in pairs(perms) do
		if not vRP.terPemissao(user_id, v) then
			return false
		end
	end
	return true
		...
		for k,v in pairs(selectors) do
			...
			if gcfg and menu then
				...
				local function selector_enter(source)
					local user_id = vRP.getUserId(source)
					if user_id and vRP.terPemissaos(user_id,gcfg.permissions or {}) then
